...
RSpec.describe Lutaml::UmlRepository::Presenters::DiagramPresenter do
  ...
  describe "#initialize" do
    ...
    it "accepts config_path option" do
      presenter_with_config = described_class.new(diagram, repository, config_path: "custom/config.yml")
  ...
  describe "#svg_output" do
    ...
    it "passes config_path to renderer" do
      presenter_with_config = described_class.new(diagram, repository, config_path: "test/config.yml")
      allow(diagram).to receive(:diagram_objects).and_return([])
      allow(diagram).to receive(:diagram_links).and_return([])
  ...
  describe "#to_text" do
    ...
    it "includes connector count" do
      allow(diagram).to receive(:diagram_links).and_return([double, double, double])
      text = presenter.to_text
  ...
  describe "private methods" do
    describe "#build_elements_data" do
      context "with diagram_objects" do
        ...
        before do
          allow(diagram).to receive(:diagram_objects).and_return([diagram_object])
    ...
    describe "#build_connectors_data" do
      context "with diagram_links" do
        ...
        before do
          ...
          allow(repository).to receive(:associations_index).and_return([mock_association])
    ...
    describe "#parse_diagram_link_style" do
      it "parses SOID from style string" do
        result = presenter.send(:parse_diagram_link_style, "SOID=12345;EOID=67890;")
      ...
      it "ignores unknown properties" do
        result = presenter.send(:parse_diagram_link_style, "UNKNOWN=999;SOID=123;")
    ...
    describe "#extract_ea_id" do
      ...
      it "returns nil for missing style" do
        ...
        allow(object_without_style).to receive(:respond_to?).with(:style).and_return(false)
    ...
    describe "#find_element_by_xmi_id" do
      ...
      it "returns nil when repository is nil" do
        ...
        element = presenter_without_repo.send(:find_element_by_xmi_id, "CLASS_001")
        expect(element).to be_nil
      end
    end
    ...
    describe "#find_connector_by_xmi_id" do
      ...
      before do
        allow(repository).to receive(:associations_index).and_return([mock_association])
        allow(repository).to receive(:classes_index).and_return([mock_class_with_gen])
        allow(mock_class_with_gen).to receive(:respond_to?).with(:generalization).and_return(true)
        allow(mock_generalization).to receive(:respond_to?).with(:xmi_id).and_return(true)
      end
      ...
      it "handles array of generalizations" do
        ...
        allow(mock_class_with_gen).to receive(:generalization).and_return(gen_array)
      ...
      it "finds association in class" do
        ...
        mock_class_with_assoc = double("Class", associations: [mock_class_assoc])
        allow(repository).to receive(:classes_index).and_return([mock_class_with_assoc])
        allow(mock_class_with_assoc).to receive(:respond_to?).with(:generalization).and_return(false)
        allow(mock_class_with_assoc).to receive(:respond_to?).with(:associations).and_return(true)
        allow(mock_class_assoc).to receive(:respond_to?).with(:xmi_id).and_return(true)
    ...
    describe "#find_connector_target" do
      let(:elements_map) { { "TARGET_ID" => { id: "TARGET_ID", name: "Target" } } }
      ...
      it "uses supplier property when available" do
        ...
        allow(connector).to receive(:respond_to?).with(:target).and_return(false)
        allow(connector).to receive(:respond_to?).with(:supplier).and_return(true)
      ...
      it "uses general property when available" do
        ...
        allow(connector).to receive(:respond_to?).with(:target).and_return(false)
        allow(connector).to receive(:respond_to?).with(:supplier).and_return(false)
        allow(connector).to receive(:respond_to?).with(:general).and_return(true)
      ...
      it "uses second member_end when available" do
        ...
        allow(connector).to receive(:respond_to?).with(:target).and_return(false)
        allow(connector).to receive(:respond_to?).with(:supplier).and_return(false)
        allow(connector).to receive(:respond_to?).with(:general).and_return(false)
        allow(connector).to receive(:respond_to?).with(:member_end).and_return(true)
    ...
    describe "#find_connector_source" do
      let(:elements_map) { { "SOURCE_ID" => { id: "SOURCE_ID", name: "Source" } } }
      ...
      it "uses client property when available" do
        ...
        allow(connector).to receive(:respond_to?).with(:source).and_return(false)
        allow(connector).to receive(:respond_to?).with(:client).and_return(true)
      ...
      it "uses specific property when available" do
        ...
        allow(connector).to receive(:respond_to?).with(:source).and_return(false)
        allow(connector).to receive(:respond_to?).with(:client).and_return(false)
        allow(connector).to receive(:respond_to?).with(:specific).and_return(true)
      ...
      it "uses owner_end property when available" do
        ...
        allow(connector).to receive(:respond_to?).with(:source).and_return(false)
        allow(connector).to receive(:respond_to?).with(:client).and_return(false)
        allow(connector).to receive(:respond_to?).with(:specific).and_return(false)
        allow(connector).to receive(:respond_to?).with(:owner_end).and_return(true)
      ...
      it "uses first member_end when available" do
        ...
        allow(connector).to receive(:respond_to?).with(:client).and_return(false)
        allow(connector).to receive(:respond_to?).with(:specific).and_return(false)
        allow(connector).to receive(:respond_to?).with(:owner_end).and_return(false)
        allow(connector).to receive(:respond_to?).with(:member_end).and_return(true)
    ...
    describe "#extract_stereotype" do
      it "extracts stereotype string" do
        ...
        allow(element).to receive(:respond_to?).with(:stereotype).and_return(true)
      ...
      it "returns nil when not available" do
        ...
        allow(element).to receive(:respond_to?).with(:stereotype).and_return(false)
    ...
    describe "#extract_attributes" do
      ...
      it "extracts array of attribute data" do
        ...
        allow(element).to receive(:respond_to?).with(:attributes).and_return(true)
        allow(mock_attribute).to receive(:respond_to?).with(:visibility).and_return(true)
      ...
      it "returns empty array when not available" do
        element = double("Element")
        allow(element).to receive(:respond_to?).with(:attributes).and_return(false)
    ...
    describe "#extract_operations" do
      ...
      it "extracts array of operation data" do
        ...
        allow(element).to receive(:respond_to?).with(:operations).and_return(true)
        allow(mock_operation).to receive(:respond_to?).with(:visibility).and_return(true)
        allow(mock_operation).to receive(:respond_to?).with(:return_type).and_return(true)
        allow(mock_operation).to receive(:respond_to?).with(:owned_parameter).and_return(true)
      ...
      it "returns empty array when not available" do
        element = double("Element")
        allow(element).to receive(:respond_to?).with(:operations).and_return(false)
    ...
    describe "#extract_parameters" do
      ...
      it "extracts array of parameter data" do
        ...
        allow(operation).to receive(:respond_to?).with(:owned_parameter).and_return(true)
      ...
      it "returns empty array when not available" do
        ...
        allow(operation).to receive(:respond_to?).with(:owned_parameter).and_return(false)
  ...
  describe "DiagramRendererWrapper" do
    ...
    describe "#initialize" do
      ...
      it "handles empty elements" do
        ...
        empty_wrapper = described_class::DiagramRendererWrapper.new(empty_data, layout_engine)
        expect(empty_wrapper.elements).to eq([])
      end

      it "handles nil elements" do
        nil_data = { name: "Nil", elements: nil, connectors: nil }
        nil_wrapper = described_class::DiagramRendererWrapper.new(nil_data, layout_engine)
