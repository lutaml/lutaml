      ...
      class DiagramCommand
        ...
        def convert_diagram_to_rendering_format(diagram, repository)
          ...
          diagram.diagram_objects.each do |obj|
            ...
            uml_element = find_uml_element_by_xmi_id(obj.object_xmi_id, repository)
            ...
            if uml_element.respond_to?(:stereotype) && uml_element.stereotype
              element_data[:stereotype] = uml_element.stereotype.is_a?(Array) ? uml_element.stereotype.first : uml_element.stereotype
            end
          ...
          diagram.diagram_links.each do |link|
            # Find the actual connector that this diagram link represents
            connector = find_connector_by_xmi_id(link.connector_xmi_id, repository)
            ...
            if connector.respond_to?(:owner_end_attribute_name) && connector.owner_end_attribute_name
              connector_data[:source_role] = connector.owner_end_attribute_name
            end

            if connector.respond_to?(:member_end_attribute_name) && connector.member_end_attribute_name
            ...
            if connector.respond_to?(:source) && connector.source
              source_obj = find_diagram_object_for_element(connector.source.xmi_id, diagram)
              if source_obj
                connector_data[:source_x] = source_obj.left + (source_obj.right - source_obj.left) / 2
                connector_data[:source_y] = source_obj.top + (source_obj.bottom - source_obj.top) / 2
              end
            end

            if connector.respond_to?(:target) && connector.target
              target_obj = find_diagram_object_for_element(connector.target.xmi_id, diagram)
              if target_obj
                connector_data[:target_x] = target_obj.left + (target_obj.right - target_obj.left) / 2
                connector_data[:target_y] = target_obj.top + (target_obj.bottom - target_obj.top) / 2
              end
            end
        ...
        def find_uml_element_by_xmi_id(xmi_id, repository)
        ...
        def render_diagram(diagram_id)
          ...
          begin
            repository = Lutaml::UmlRepository::Repository.from_package(options[:lur_path] || "examples/lur/basic.lur")
            ...
            unless diagram
              ...
              diagram = all_diagrams.find { |d| d.name.downcase.include?(diagram_id.downcase) }
            end
            ...
            diagram_data = convert_diagram_to_rendering_format(diagram, repository)

            # Render using the diagram module
            svg_content = Lutaml::Ea::Diagram.render(diagram_data, options)

            # Write to output file
            output_path = options[:output] || "#{diagram.name.gsub(/[^a-zA-Z0-9]/, '_')}.svg"
            ...
            elements: [
              {
                ...
                operations: [
                  ...
                  { name: "setName", parameters: [{ name: "name", type: "String" }], visibility: "public" }
                ]
              },
              {
                id: "class2",
